...
import { getPlaylistIcons } from "./api.js";
// Кадры-превью общие с режимом плитки — см. thumbs.js
import { thumbUrl, hasThumb, captureThumb } from "./thumbs.js";
...
export function buildBannerEl(name, data, { coubMap, animate = true, cacheBust } = {}) {
    ...
    const stillSrc = data?.banner?.image
        ...
        : legacyIconExists(name) ? legacyIconUrl(name) + bust
            : hasThumb(coubId) ? thumbUrl(coubId)
                : null;
