...
//
// Раньше это были два файла, которые плеер читал прямо из Data/ — теперь
// данные живут в базе, и те же самые структуры отдаёт API. Форма ответов
// не изменилась: менять её заодно с хранилищем значило бы чинить потом
// сразу две вещи, не зная, которая сломалась.
//
// Пустой ответ (первый запуск, пока ничего не скачано и не создано) — это
// не ошибка, а нормальное состояние: возвращаем пустые структуры, чтобы
// плеер мог продолжить работу, а не падал целиком.
...
export async function loadData() {
    const [playlists, coubs] = await Promise.all([
        fetchJsonSafe(`/api/playlists?t=${Date.now()}`, {}),
        fetchJsonSafe(`/api/coubs/list?t=${Date.now()}`, []),
    ]);
